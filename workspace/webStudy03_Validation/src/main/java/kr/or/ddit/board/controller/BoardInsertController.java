...
public class BoardInsertController {
	...
	//마스킹처리(욕설)
	public String filteringTokens(String origin, String...tokens) {
//		"말미잘", "해삼". "멍게"
		String regex = String.format("(%s)", String.join("|", tokens));
		Pattern patter = Pattern.compile(regex);
		Matcher matcher  =patter.matcher(origin);
		StringBuffer result = new StringBuffer();
		while(matcher.find()) {
			matcher.appendReplacement(result, "AAA");
		}
		// 마지막 말 이후의 텍스트 처리
	...
	public String insert(
		...
		if(bo_files != null) {
			List<AttatchVO> attatch = new ArrayList<>();
			for(MultipartFile file:bo_files) {
				if(file.isEmpty()) continue;
				attatch.add(new AttatchVO(file));
			}
			if(attatch.size()>0) {
				board.setAttatchList(attatch);
			}
		}
		...
		String replaceText = RegexUtils.filteringTokens(board.getBo_content(), "X", filteringTokens);
		board.setBo_content(replaceText);
		ServiceResult result =  service.createBoard(board);
		switch (result) {
		case PKDUPLICATED:
			view = "board/boardInsert.do";
			message = "아이디 중복";
			break;
		case OK:
