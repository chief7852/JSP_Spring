...
<pre>
   ...
   //      response.addCookie(allDomain);
      ...
      5. 요청에 포함된 쿠키를 꺼내서 상태 복원 : 
         <a href="cookieView.jsp">쿠키경로확인하기(동일경로)</a>
         <a href="../08/cookieView.jsp">쿠키경로확인하기(다른경로)</a>

   ** 쿠키 속성의 종류
   1. name(required)
   2. value(requried) : 특수문자를 포함하는 경우, REC2396 규약에 따라 url encoding 방식을 사용함.
   3. domain(host) : 쿠키의 재전송을 결정하는 조건.
      생략시, 쿠키 생성 도메인이 기본값으로 반영됨
      ex) .naver.com : host 에 상관없이 재전송 가능/
   4. path : 쿠키의 재전송을 결졍하는 조건
      다음번 요청이 설정된 경로 이하로 발생할때만 재전송.
      생략시, 쿠키 생성시의 경로가 기본값으로 반영됨.
   5. maxAge(exoures) : 쿠키의 만료 시점 (second)
      생략시, 기본값으로 세션과 동일한 만료 시점이 사용.
      0  : 쿠키 삭제(쿠키의 모든 조건이 동일)
      -1 : 브라우저 종료시 크키 삭제 
